...
export default class Geocoder {
  constructor(options) {
    ...
    this.httpOptions = {
      ...
      headers: {
        "User-Agent":
          getPropValue(options, "userAgent") || "node-open-geocoder",
      },
    };
  ...
  reverse(lon, lat) {
    if (!isValidCoordinates(lon, lat)) {
      ...
      this.httpOptions.path = `/reverse?lon=${lon}&lat=${lat}${this.format}&addressdetails=1`;
    }
...
function responseHandler(handler, res) {
  ...
  function onEnd() {
    ...
    return json.error ? handler(json.error) : handler(null, json.value);
  }
}
